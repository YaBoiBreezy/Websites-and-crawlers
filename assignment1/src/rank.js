...
export default async function computePageRank(webIndex) {
  try {
    let pages = await db.page.findMany({
      where: {
        web: webIndex, //get pages from the right web
      },
    });

    let pageIds = pages.map((page) => page.id); //map the page id's so I only get links for those pages
    let links = await db.link.findMany({
      where: {
        OR: pageIds.map((id) => ({ sourceId: id })),
      },
    });
